...
def test_imports():
    ...
    try:
        ...
        logger.info("âœ“ parse_mat imported")
        
        from src.data.pairing import pair_discharge_eis, validate_pairs
        logger.info("âœ“ pairing imported")
        
        from src.data.split import split_by_battery, get_split_statistics
        logger.info("âœ“ split imported")
        
        from src.data.dataset import create_dataloaders
        logger.info("âœ“ dataset imported")
        
        import torch
        logger.info(f"âœ“ torch {torch.__version__} imported")
...
def test_data_parsing():
    ...
    try:
        ...
        logger.info(f"âœ“ Parsed {len(discharge_df)} discharge records")
        logger.info(f"âœ“ Parsed {len(eis_df)} EIS records")
        ...
        logger.error(f"âœ-- Data parsing failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_pairing(discharge_df, eis_df):
    ...
    try:
        ...
        logger.info(f"âœ“ Created {len(paired_df)} pairs")
        
        if len(paired_df) == 0:
            logger.error("âœ-- No pairs created!")
            return False
        ...
        logger.error(f"âœ-- Pairing failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_split(paired_df):
    ...
    try:
        ...
        logger.info(f"âœ“ Train: {len(train_df)} samples")
        logger.info(f"âœ“ Val: {len(val_df)} samples")
        logger.info(f"âœ“ Test: {len(test_df)} samples")
        ...
        logger.error(f"âœ-- Split failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_model():
    ...
    try:
        ...
        logger.info(f"âœ“ BaFuse model created")
        ...
        logger.info(f"âœ“ Forward pass works")
...
def main():
    ...
    logger.info("\n")
    logger.info("â•”" + "="*78 + "â•--")
    logger.info("â•‘" + " "*20 + "BaFuse Training Pipeline Test" + " "*29 + "â•‘")
